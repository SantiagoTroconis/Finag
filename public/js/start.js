...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadVideos() {
        fetch('/videos.json')
            .then(res => res.json())
            .then(videos => {
                const highlightsContainer = document.getElementById('highlightsContainer').querySelector('.horizontal-scroll-wrapper .row');
                let videosHTML = '';
                                ...
                                <div class="col-custom">
                                    <div class="card h-100">
                                        ...
                                        <div class="filter-buttons text-center">
                                            <button class="filter-btn active" data-filter="normal" data-video-id="${video.videos[1].titulo.replace(/\s+/g, '-').toLowerCase()}">
                                                <i class="bi bi-circle"></i> Normal
                                            </button>
                                            <button class="filter-btn" data-filter="grayscale" data-video-id="${video.videos[1].titulo.replace(/\s+/g, '-').toLowerCase()}">
                                                <i class="bi bi-moon"></i> B&N
                                            </button>
                                            <button class="filter-btn" data-filter="sepia" data-video-id="${video.videos[1].titulo.replace(/\s+/g, '-').toLowerCase()}">
                                                <i class="bi bi-sun"></i> Sepia
                                            </button>
                                        </div>
                                    </div>
                                </div>
                ...
                highlightsContainer.innerHTML = videosHTML;
    ...
    async function loadTFModel() {
        ...
        resultEl.textContent = "Modelo TendsorFlow cargado";
    }

    async function predictFrame() {
        if (!model || isProcessing || !video.srcObject) {
        ...
        isProcessing = false;
        predictionRAF = requestAnimationFrame(predictFrame);
    }
    ...
    async function toggleARModel() {
        ...
        if (currentModelInScene) {
            ...
            try {
                ...
                arToggleButton.disabled = false; // Reactivar el botón
            }
        }
    }
    ...
    function applyModelScale(model, country) {
        let scale = 2.0;
        // ["Canada", "Mexico", "Usa", "Argentina", "Brasil", "Colombia", "Paraguay", "Ecuador", "Uruguay", "Australia", "Portugal", "Japon", "Jordania", "Corea del sur", "España", "Arabia saudita", "Marruecos", "Francia", "Sin bandera"];
    ...
    function disposeModel(model) {
        ...
        console.log("Limpiando modelo de memoria...");
